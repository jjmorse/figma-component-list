...
const componentList = componentNodes.map(node => '<li>' + node.name + (node.remote ? ' (library)' : '') + '</li>').join('');
const instanceList = instanceNodes.map(node => '<li>' + node.mainComponent.name + (node.mainComponent.remote ? ' <b><i>(library)</i></b>' : '') + '</li>').join('');
